...
def main():
    ...
    # for i in servo_ids_to_check:
        # servo_id = node.board.bus_servo_read_id(i)
        # if servo_id != None:
            # node.get_logger().info("Detected serial servo ID:%d " % (servo_id[0],))
        # else:  # 没有在总线上找到该舵机(the servo was not found on the bus)
            # msg = "Could not detect serial servo ID:{} on the bus!!! Node is shutting down.".format(i)

            # while True:
                # node.board.set_buzzer(2000, 0.05, 0.01, 1)
                # time.sleep(0.2)
